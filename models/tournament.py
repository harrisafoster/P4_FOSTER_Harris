...
class Tournament:
    ...
    def create_tournament(self, name_of_tournament, location, nb_rounds,
    ...
    def access_tournament_object(self, name_of_tournament):
        tournament_instance = self.db_tournaments.search(
    ...
    def read_one_tournament(self, name_of_tournament):
        return self.db_tournaments.search(
            self.query.name_of_tournament == name_of_tournament)

    def delete_tournament(self, name_of_tournament):
        self.db_tournaments.remove(
            where('name_of_tournament') == name_of_tournament)
    ...
    def check_if_already_played(self, new_match):
        if new_match in list(itertools.chain.from_iterable(
                self.round_descriptions)):
            return True
        else:
            return False
    ...
    def sort_players_by_points_descending(self):
        self.player_instances.sort(key=operator.itemgetter(
            'local_player_index'))
        self.player_instances.sort(key=operator.itemgetter(
            'points'), reverse=True)
        return self.player_instances
    # player instances refers to the local player entry
    # within the tournament entry
    ...
    def generate_round_1_matches(self):
        ...
        bottom_half = local_player_index_numbers[
                      int(nb_players / 2):nb_players]
        round_matches = []
        for local_player_index in top_half:
            round_matches.append((local_player_index,
                                  bottom_half[local_player_index - 1]))
        ...
        for player_a in local_player_index_numbers:
            if player_a not in list(itertools.chain.from_iterable(matches)):
                for player_b in local_player_index_numbers:
                    if player_b != player_a and player_b not in list(
                            itertools.chain.from_iterable(matches)):
                        if not self.check_if_already_played(
                                (player_a, player_b))\
                                and not self.check_if_already_played(
                                [player_a, player_b])\
                                and not self.check_if_already_played(
                                (player_b, player_a))\
                                and not self.check_if_already_played(
    ...
    def swiss_method_pairing(self):
        ...
        local_player_index_numbers = self.get_local_player_index_numbers()
        new_matches = self.make_new_matches_for_swiss(
                                                local_player_index_numbers)
        if len(new_matches) == len(local_player_index_numbers) / 2:
            ...
            if len(new_matches) == len(local_player_index_numbers) / 2:
                ...
                local_player_index_numbers = \
                    (self.get_local_player_index_numbers())
                new_matches = self.make_new_matches_for_swiss(
                                                local_player_index_numbers)
                if len(new_matches) == len(local_player_index_numbers) / 2:
                    self.round_descriptions += [new_matches]
    ...
    # creates local player instances
    # within the tournament object/database entry for use
    ...
    def delete_round(self, round_number):
        ...
        self.match_instances = list(filter(lambda i: i['round_number'] !=
                                           round_number,
                                           self.match_instances))
